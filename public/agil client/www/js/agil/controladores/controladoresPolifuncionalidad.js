angular.module('agil.controladores')
    .controller('controladorPolifuncionalidad', ['$scope', '$location', '$localStorage', '$templateCache', '$route', 'blockUI', 'Paginator', 'FieldViewer', '$timeout',
        ...
        $scope.editar = function (evaluacion, editar, nuevo, ver) {
            ...
            if (ver) {
                $scope.evaluacion = {
                    ...
                    id_ficha: evaluacion.id_ficha,
                    ficha: evaluacion.ficha
                }
            ...
            if (nuevo) {
                ...
                $scope.evaluacion.id_ficha = evaluacion.id_ficha;
                $scope.evaluacion.ficha = evaluacion.ficha;
        ...
        $scope.reportePorMeses = function (fromMonth, fromYear, untilMonth, untilYear, pdf, grafico) {
            ...
            reporte.then(function (res) {
                ...
                for (var i = 0; i < res.reporte.length; i++) {
                    let ficha= res.reporte[i];
                    var columns = [];
                    columns.push(i + 1);
                    columns.push(ficha.empleado.campo.nombre);
                    columns.push(ficha.empleado.persona.nombre_completo);
                    columns.push(ficha.empleado.persona.ci);
                    columns.push($scope.determinarEstado(ficha.empleado.eliminado));
                    columns.push(ficha.cargos[0]);
                    var currentIndx = 0
                    var promedio = 0
                    mesesReporte.map(function (mes) {
                        var fechaCabecera = currentIndx <= ficha.evaluaciones.length - 1 ? $scope.meses[ficha.evaluaciones[currentIndx].mes].nombre.substring(0, 3) + '-' + (ficha.evaluaciones[currentIndx].anio).toString().substring(4, 2) : "0"
                        if (mes == fechaCabecera) {
                            columns.push((ficha.evaluaciones[currentIndx].nota_total))
                            promedio += ficha.evaluaciones[currentIndx].nota_total
                            currentIndx += 1
                        } else {
                            columns.push("-")
                        }
                    ...
                    promedio = promedio / ficha.evaluaciones.length
                ...
                var men = (err.stack !== undefined && err.stack !== null) ? err.stack : err.message
                $scope.mostrarMensaje('Se produjo un error!<br />' + men)
                blockUI.stop();
            })
        }
        ...
        $scope.reportePorMesesPdf = function (reporte, mesDesde, anioDesde, mesHasta, anioHasta) {
            ...
            convertUrlToBase64Image($scope.usuario.empresa.imagen, function (imagenEmpresa) {
                ...
                for (var i = 0; i <= (reporte.length - 1); i++) {
                    let datosReporte = reporte[i]
                    if (datosReporte.length <= 14) {
                        if (i == 0) {
                        ...
                        for (var j = 0; j < datosReporte.length; j++) {
                            let detalle =datosReporte[j]
                            if (i > 0) {
                            ...
                            if (j == 0) {
                                doc.text(detalle, x, y, { width: limiteChar });
                                x = x - 30
                            }
                            if (j > 0 && j < 5) {
                                if ((j == 2 || j == 1 || j == 5) && i >= 1) {
                                ...
                                doc.text(detalle, x, y, { width: limiteChar, align: "center" });
                            }
                            if (j > 4 && j < datosReporte.length - 3) {
                                if (j == 5) {
                                    let cargo = detalle?.cargo?detalle.cargo.nombre:detalle
                                    doc.text(cargo.toLowerCase(), x, y, { width: limiteChar, align: "center" });
                                } else {
                                    doc.text(detalle, x, y, { width: limiteChar, align: "center" });
                                }
                            ...
                            if (j >= datosReporte.length - 3) {
                                doc.text(detalle, x, y, { width: limiteChar, align: "center" });
                            }
                            x += spaceX
                        }
                    }
                    if (datosReporte.length >= 15 && datosReporte.length <= 23) {
                        if (i == 0) {
                            doc.font('Helvetica-Bold', 12);
                            doc.text("MATRIZ POLIFUNCIONALIDAD Y DESEMPEÑO", 50, 30, { align: "center" });
                        ...
                        for (var j = 0; j < datosReporte.length; j++) {
                            ...
                            if (j > 0 && j < 5) {
                                ...
                                doc.text(detalle, x, y, { width: limiteChar - 5, align: "center" });
                                x = x - 5
                            }
                            if (j > 4 && j < datosReporte.length - 3) {
                                doc.text(detalle, x, y, { width: limiteChar - 5, align: "center" });
                    ...
                    if (y > 700) {
                        ...
                        for (var k = 0; k < reporte[0].length; k++) {
                            ...
                            if (k > 4 && k < datosReporte.length - 3) {
                                x = x - 10
                            }
        ...
        $scope.obtenerEvaluaciones = function () {
            ...
            prom.then(function (res) {
                $scope.evaluaciones = res.evaluaciones
        ...
        $scope.establecerPersonal = function (personal, modal) {
            ...
            let ficha = Object.assign(personal.empleadosFichas[0]);
            ficha.empleado =personal;
            delete ficha.empleado.empleadosFichas;
            $scope.evaluacion.ficha = ficha
