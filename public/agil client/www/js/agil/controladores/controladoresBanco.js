angular.module('agil.controladores')
	...
	.controller('ControladorBancos',['$scope', '$localStorage', '$location', '$templateCache', '$route', 'blockUI',
		'ClasesTipo', 'Clases', 'Banco', 'ListaBancos', 'BancoDatos', 'BancoPaginador', '$window', 'ActivarBanco','ListaCuentasComprobanteContabilidad', function ($scope, $localStorage, $location, $templateCache, $route, blockUI,
		ClasesTipo, Clases, Banco, ListaBancos, BancoDatos, BancoPaginador, $window, ActivarBanco,ListaCuentasComprobanteContabilidad ) {
		blockUI.start();
		...
		$scope.activarBanco = function (estado, banco) {
			blockUI.start();
			banco.eliminar = estado;
			ActivarBanco.update({ id_banco: banco.id }, banco, function (res) {
				$scope.mostrarMensaje('Banco actualizado Exitosamente!');
			}, function (error) {
		...
		$scope.saveForm = function (banco) {
			var button=$('#modal-wizard-banco').find('button[type=submit]').text().trim();
			if (button != "Siguiente") {
				blockUI.start();
				if (banco.id) {
