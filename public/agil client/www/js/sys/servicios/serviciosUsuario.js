angular.module('agil.servicios')
	...
	.factory('VerificarAutentificacion', ['$resource', function ($resource) {
		return $resource(restServer + "verificar/autenticacion", {},
			{
				'update': { method: 'PUT' }
			});
	}])
	...
	.factory('VerificarAutentificacionUsuarios', ['VerificarAutentificacion','$window', 'IndexDbVerificarAutentificacionUsuarios', '$q', function (VerificarAutentificacion,$window, IndexDbVerificarAutentificacionUsuarios, $q) {
		var res = function () {
			const online = $window.navigator.onLine;
			if(!online){
				return IndexDbVerificarAutentificacionUsuarios();
			}
			var delay = $q.defer();
			VerificarAutentificacion.save({}, function (entidades) {
	...
	.factory('VistaColumnasAplicacion', ['$resource', function ($resource) {
		return $resource(restServer + "columnnas-aplicacion/aplicacion/:id_aplicacion/:nombre?", { id_aplicacion: '@id' },
			{
				'update': { method: 'PUT' }
			});
	}])
	.factory('VerificarUsuarioEmpresaRuta', ['$resource', function ($resource) {
		return $resource(restServer + "verificar/usuarios/empresa/:id_empresa", { id_empresa: '@id_empresa' },
			{
				'update': { method: 'PUT' }
			});
	}])
	.factory('VerificarUsuarioEmpresa', ['VerificarUsuarioEmpresaRuta', '$q', function (VerificarUsuarioEmpresaRuta, $q) {
		var res = function (idEmpresa, cuenta) {
			var delay = $q.defer();
			VerificarUsuarioEmpresaRuta.save({ id_empresa: idEmpresa }, cuenta, function (entidad) {
