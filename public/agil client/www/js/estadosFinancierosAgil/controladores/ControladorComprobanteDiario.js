	...
	.controller('ControladorComprobanteDiario', function ($scope, $localStorage, $location, $templateCache, $route, blockUI,
		...
		$scope.imprimirReportePDF = function(inicio,fin){
            ...
            promesa.then(function(datos){                  
				...
				var suc = $scope.datosReporte[0].sucursal.nombre;
				var direccion = $scope.datosReporte[0].sucursal.empresa.direccion;
				empresa = {razon_social,nit, suc,direccion};

				var doc = new PDFDocument({ compress: false, margin: 10 });
				var stream = doc.pipe(blobStream());
				doc.font('Helvetica', 8);
				var y = 140, itemsPorPagina = 30, items = 0, pagina = 1;
				var cant = 0;
				...
				var indice = 1;
				for (var i = 0; i < $scope.datosReporte.length; i++) {
					...
					doc.text(fecha,40,y, {width: 40, align:'right'});
					doc.text("- "+(indice++)+" -",90,y, {width: 312, align:'center'});
					doc.text($scope.comprobante.numero,402,y,{width:40, align: 'center'});
					doc.font('Helvetica');
					y = y + 10;
					for(var j = 0;j< $scope.comprobante.asientosContables.length;j++){
						$scope.asientoContable = $scope.comprobante.asientosContables[j];
						if($scope.asientoContable.debe_bs > 0){
							doc.text($scope.asientoContable.cuenta.nombre,90,y,{width:312, underline:true});
						}else{
							doc.text($scope.asientoContable.cuenta.nombre,110,y,{width:292, underline:true});	
						}
						...
						if($scope.asientoContable.debe_bs != 0){
							doc.text($scope.asientoContable.debe_bs,442,y, {width: 70, align: 'right'});
						}else{
							doc.text(" ",442,y);
						}
						if($scope.asientoContable.haber_bs != 0){
							doc.text($scope.asientoContable.haber_bs,512,y, {width: 70, align: 'right'});
						}else{
							doc.text(" ",512,y);
						}
						/*}*/
					...
					doc.text($scope.comprobante.gloza,90,y,{width: 312});
					y = y + 25;
					if (y >= 730) {	
						doc.addPage({ margin: 0, bufferPages: true });
						y = 140;
...
*/
		...
		$scope.dibujarCabeceraPDFReporteGestion = function (doc, datos, pagina, totalPaginas) {
			...
			doc.text(datos.razon_social,0,30, {width: 612, align: 'center'});
			doc.text("NIT: "+datos.nit,0,40, {width: 612, align: 'center'});
			doc.text(datos.suc,0,55, {width: 612, align: 'center'});
			doc.text(datos.direccion,0,65, {width: 612, align: 'center'});
			doc.text("Santa Cruz - Bolivia",0, 75, {width: 612, align: 'center'});
			
			doc.font("Helvetica-Bold",15);
			doc.text("LÍBRO DIARIO",0,100,{ width: 612,align: "center" });

			doc.font("Helvetica-Bold",10);
			doc.text("FECHA",40,120,{width: 50, align:'center'});
			doc.text("CUENTAS ",90,120,{width: 312, align:'center'});
			doc.text("N° Doc",402,120,{width: 40, align:'center'});
			doc.text("DEBE",442,120,{width: 70, align:'right'});
			doc.text("HABER",512, 120,{width: 70, align:'right'});
			...
			doc.text("Impresión: " + fechaActual.getDate() + "/" + (fechaActual.getMonth() + 1) + "/" + fechaActual.getFullYear() + " Hrs." + fechaActual.getHours() + ":" + min, 30, 750);
			doc.text("Usuario: " + $scope.usuario.nombre_usuario, 135, 750);
			doc.text("Página " + pagina, 552, 750);
		}
