module.exports=function(sequelize,Sequelize){
	...
	Clase.sync().then(function(){
		...
		sequelize.query("INSERT IGNORE INTO gl_clase SET id = 19,tipo=5,nombre = 'ALMACÉN',nombre_corto='ALM',createdAt = NOW(),updatedAt = NOW();").spread(function(results, metadata) {
		...
		sequelize.query("INSERT IGNORE INTO gl_clase SET id = 21,tipo = 6,nombre = 'FACTURAS',nombre_corto = 'ID',createdAt = NOW(),updatedAt = NOW();").spread(function(results, metadata) {
		...
		sequelize.query("INSERT IGNORE INTO gl_clase SET id = 2214,tipo = 6,nombre = 'IMPORTACIONES',nombre_corto = 'IPI',createdAt = NOW(),updatedAt = NOW();").spread(function(results, metadata) {
		  
		});
		sequelize.query("INSERT IGNORE INTO gl_clase SET id = 2215,tipo = 6,nombre = 'RETENCIÓN DE SERVICIOS',nombre_corto = 'IPRS',createdAt = NOW(),updatedAt = NOW();").spread(function(results, metadata) {
		  
		});
		sequelize.query("INSERT IGNORE INTO gl_clase SET id = 2216,tipo = 6,nombre = 'RETENCIÓN DE BIENES',nombre_corto = 'IPRB',createdAt = NOW(),updatedAt = NOW();").spread(function(results, metadata) {
		  
		});
		sequelize.query("INSERT IGNORE INTO gl_clase SET id = 2217,tipo = 6,nombre = 'COMPRAS S/F',nombre_corto = 'IPCSF',createdAt = NOW(),updatedAt = NOW();").spread(function(results, metadata) {
		  
		});
		sequelize.query("INSERT IGNORE INTO gl_clase SET id = 14633,tipo = 245,nombre = 'PROVEEDOR',nombre_corto = 'PROVEEDOR',createdAt = NOW(),updatedAt = NOW();").spread(function(results, metadata) {
		  
		});

		sequelize.query("INSERT IGNORE INTO gl_clase SET id = 17707,tipo = 6,nombre = 'INGRESO COMPROBANTES',nombre_corto = 'IC',createdAt = NOW(),updatedAt = NOW();").spread(function(results, metadata) {
		  
		});
		sequelize.query("INSERT IGNORE INTO gl_clase SET id = 17708 ,tipo = 6,nombre = 'INGRESO CAJA CHICA',nombre_corto = 'ICAJCH',habilitado = true, eliminado = false,createdAt = NOW(),updatedAt = NOW();").spread(function(results, metadata) {
		  
		});

		sequelize.query("INSERT INTO gl_clase (tipo,nombre,nombre_corto,habilitado,eliminado,createdAt,updatedAt) SELECT * FROM \
            ( SELECT (SELECT gl_tipo.id FROM gl_tipo WHERE gl_tipo.nombre_corto = 'CM_CCH') AS tipo, 'CLIENTE' AS nombre, 'CLIENTE' AS nombre_corto, 1 AS habilitado, 0 AS eliminado, '2021-07-08 00:00:00' AS createdAt, '2021-07-08 00:00:00' AS updatedAt ) AS temp WHERE\
            NOT EXISTS ( SELECT nombre_corto FROM gl_clase WHERE nombre_corto = 'CLIENTE' ) LIMIT 1;").spread(function(results, metadata) {
