module.exports = function (router, ensureAuthorizedAdministrador, fs, forEach, jwt, md5, GtmDespacho,
	...
	router.route('/alertas/pedidos/cantidad/:id_empresa')
	.get(function (req, res) {
		...
		GtmDespachoDetalle.count({
			...
			res.json({cantidad_pedidos: detallesDespacho});
		});
	});
	...
	function calcularCostosEgresos(tipoMovimiento, tipoEgreso, producto, cantidad, inventarios, index, array, res, t, req, detalle_despacho, numero) {
		...
		if (producto.dataValues.activar_inventario) {
			...
			// }
			return new Promise(function (fulfill, reject) {
				reject("el producto <b>"+producto.nombre+"</b> no tiene activo el inventario")
			});
		}
		}
		...
		router.route('/gtm-detalle-despacho/empresa/:id_empresa/fecha/:fecha')
			.put(function (req, res) {
				...
				sequelize.transaction(function (t) {
					...
					if (req.body.mensaje != "") {
						res.json({sinGuardar:true, mensaje: "de los siguientes clientes no se realizaron los despachos por falta de inventario o fue eliminado:                                                " + req.body.mensaje });
					} else {
		...
		router.route('/gtm-detalle-despacho-despachado/empresa/:id_empresa/pagina/:pagina/items-pagina/:items_pagina/busqueda/:texto_busqueda/columna/:columna/direccion/:direccion/inicio/:inicio/fin/:fin/transportista/:transportista/tipo/:tipo/grupo/:grupo/estado/:estado/vendedor/:vendedor/admin/:admin')
			.get(function (req, res) {
				...
				if (req.params.inicio != 0) {
					var inicio = new Date(req.params.inicio); inicio.setHours(4, 0, 0, 0);
					var fin = new Date(req.params.fin); fin.setHours(19, 59, 59, 0);
								...
								if (despacho != null) {
									...
									GtmVentaKardexDetalle.update({
										saldo: despacho.dataValues.detalle_Kardex.padre.saldo + despacho.dataValues.detalle_Kardex.cantidad_despachada - parseFloat(req.body.cantidad)
											...
											GtmVentaKardexDetalle.update({
												cantidad_despachada: parseFloat(req.body.cantidad),
			...
			.get(function (req, res) {
				...
				GtmDespachoDetalle.findAll({
					...
					include: [{
						...
						include: [{ model: Usuario, as: 'usuario', include: [{ model: Persona, as: 'persona', where: condicionEmpleado }] },
						{ model: Cliente, as: 'cliente', where: condicionCliente },
						{ model: ClienteRazon, as: 'cliente_razon', required: true },
						{ model: GtmDestino, as: 'destino' }]
					},
					{ model: Producto, as: 'producto' },
					{ model: GtmVentaKardexDetalle, as: 'detalle_Kardex' }]
		...
		router.route('/gtm-detalle-despacho-ubicacion/vendedor/:id_vendedor/fecha/:fecha')
			.get(function (req, res) {
				...
				var fin = new Date(req.params.fecha); fin.setHours(23, 59, 59, 0);
