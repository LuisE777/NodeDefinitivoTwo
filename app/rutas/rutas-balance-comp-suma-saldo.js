    ...
    router.route('/balance-comp-suma-saldo/empresa/:id_empresa/tipo_periodo/:periodo/gestion/:gestion/mes/:mes/inicio/:inicio/fin/:fin/gestion_fin/:gestion_fin')
        .post(ensureAuthorizedlogged, function (req, res) {
            ...
            if (req.params.periodo == 'GESTIÓN') {
                ...
                var mes = new Date(req.params.gestion, parseInt(req.params.mes), 0);
                var primerDia = new Date(req.params.gestion, parseInt(req.params.mes), 1, 0, 0, 0);
                var ultimoDia = new Date(req.params.gestion, parseInt(req.params.mes), mes.getDate(), 23, 59, 59);
                ...
                inicio.setHours(0, 0, 0, 0, 0);
                fin.setHours(23, 59, 59, 0, 0);
                condicionComprobante.eliminado = false;
            ...
            AsientoContabilidad.findAll({
                ...
                },
                {
                    ...
                    include: [
                        { model: Clase, as: 'tipoCuenta', where: { nombre: "APROPIACION" } },
                        { model: ContabilidadCuenta, as: 'cuentaPadre', required: false, include:[
                            {model: ContabilidadCuenta, as: 'cuentaPadre', required: false, include:[
                                {model: ContabilidadCuenta, as: 'cuentaPadre', required: false, include:[
                                    {model: ContabilidadCuenta, as: 'cuentaPadre', required: false}
                                ]}
                            ]}
                        ] }
                    ]
