			...
			Venta.findAll({
				...
				},
				{
					...
					include: [{
						...
						include: [{ model: RrhhEmpleadoFicha, as: 'empleadosFichas', where: { activo: true }, include: [{ model: RrhhEmpleadoCargo, as: 'cargos', include: [{ model: Clase, as: "cargo", where: condicionCargo }] }] }, { model: Persona, as: 'persona', where: condicionPersona }]
					}]
				},
	...
	router.route('/ventas-farmacia/:id/empresa/:id_empresa')
		.get(ensureAuthorizedlogged, function (req, res) {
			Venta.find({
				...
				{ model: Almacen, as: 'almacen', include: [{ model: Sucursal, as: 'sucursal', where: { activo: true } }] },
					...
					return Movimiento.create({
						...
						if (movimiento == Diccionario.EGRE_FACTURACION) {
							return SucursalActividadDosificacion.find({
								...
								transaction: t,
								include: [
									{ model: Dosificacion, as: 'dosificacion', include: [{ model: Clase, as: 'pieFactura' }] },
									{ model: Sucursal, as: 'sucursal', include: [{ model: Empresa, as: 'empresa' }] }
								]
							}).then(function (sucursalActividadDosificacion) {
								if (!sucursalActividadDosificacion. sucursal.activo) throw new Error('Sucursal deshabilitada, no se puede hacer cambios.')
								var dosificacion = sucursalActividadDosificacion.dosificacion;
